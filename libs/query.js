...
query.json._slice = function (obj, start, end, step) {
...
query.json._find = function e(obj, name) {
...
query.json._distinctFilter = function (array, callback) {
...
exports = module.exports = function (/*String*/q, /*Object?*/obj) {
    // summary:
    ...
    //		in JavaScript.
    //
    // 	|	query.json.query(queryString,object)
    //		and
    // 	|	query.json.query(queryString)(object)
    ...
    //
    // example:
    // 	|	query.json.query("foo",{foo:"bar"})
    //		This will return "bar".
    //
    // example:
    //	|	evaluator = query.json.query("?foo='bar'&rating>3");
    ...
    //
    // example:
    // 	|	evaluator = query.json.query("$[?price<15.00][\rating][0:10]");
    ...
    q.replace(/(\]|\)|push|pop|shift|splice|sort|reverse)\s*\(/, function () {
        throw new Error("Unsafe function call");
    });

    q = q.replace(/([^<>=]=)([^=])/g, "$1=$2").// change the equals to comparisons except operators ==, <=, >=
        ...
        replace(/\.?\.?\[(`\]|[^\]])*\]|\?.*|\.\.([\w\$_]+)|\.\*/g,function (t, a, b) {
            ...
            if (oper) {
                ...
                if (t.match(/^\./)) {
                    // recursive object search
                    call("query.json._find");
                    prefix = ",true)";
                }
                call(oper[1].match(/\=/) ? "array.map" : oper[1].match(/\^/) ? "query.json._distinctFilter" : "array.filter");
    ...
    q = prefix + (q.charAt(0) == '$' ? "" : "$") + q.replace(/`([0-9]+|\])/g, function (t, a) {
        //restore the strings
        return a == ']' ? ']' : str[a];
    });
    // create a function within this scope (so it can use expand and slice)

	var executor = eval("1&&function($,$1,$2,$3,$4,$5,$6,$7,$8,$9){var $obj=$;return " + q + "}");
